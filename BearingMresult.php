...
function goBack() {
   ...
   <title>BEARING DETAILS Result - Secured</title>
</head>
<body>
<?php
...
// Create connection
$conn = new mysqli(localhost, DBUWRITE_DB_USER, WRITE_DB_PASS, WRITE_DB_NAME);
// Check connection
...
$sql = "SELECT * FROM `BEARING` WHERE `MORE` LIKE '%".$_POST['id']."%'";
...
echo "</table><br><center><button class=button onclick=goBack()>TRY ANOTHER DETAILS</button></center>";
} else {
echo "<center color=red><img src=img/error.png /><br />INCORRECT DETAIL<br><button class=button onclick=goBack()>TRY ANOTHER DETAILS</button><br></center>";
}
$conn->close();
?>
