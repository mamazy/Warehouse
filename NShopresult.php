...
function goBack() {
   ...
   <title>SHOP Part Name Result - Secured</title>
</head>
<body>
<?php
...
// Create connection
$conn = new mysqli(localhost, WRITE_DB_USER, WITE_DB_PASS, WRITE_DB_NAME);
// Check connection
...
$sql = "SELECT * FROM `SHOP` WHERE `PARTNAME` LIKE '%".$_POST['id']."%'";

$result = $conn->query($sql);

if ($result->num_rows > 0) {
echo "<table id=parts><tr><th>ID</th><th>PART NAME</th><th>PART NUMBER</th><th>QTY</th><th>MORE</th></tr>";
// output data of each row
while($row = $result->fetch_assoc()) {
echo "<tr><td>".$row["ID"]."</td><td>".$row["PARTNAME"]."</td><td>".$row["PARTNUMBER"]."</td><td>".$row["QTY"]."</td><td>".$row["MORE"]."</td></tr>";
}
