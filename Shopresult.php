...
function goBack() {
   ...
   <title>SHOP Part# Result - Secured</title>
</head>
<body>
<?php
...
$sql = "SELECT * FROM `SHOP` WHERE `PARTNUMBER` LIKE '%".$_POST['id']."%'";

$result = $conn->query($sql);

if ($result->num_rows > 0) {
echo "<table id=parts><tr><th>SEQ</th><th>PART NAME</th><th>PART NUMBER</th><th>QTY</th><th>CONDITION</th><th>PRICE</th><th>EDIT</th></tr>";
// output data of each row
while($row = $result->fetch_assoc()) {
echo "<tr><td>".$row["SEQ"]."</td><td>".$row["PARTNAME"]."</td><td>".$row["PARTNUMBER"]."</td><td>".$row["QTY"]."</td><td>".$row["COND"]."</td><td>".$row["PRICE"]."</td><td><a href=\"Sedit.php?ID=$row[ID]\">Edit</a></td></tr>";
}
